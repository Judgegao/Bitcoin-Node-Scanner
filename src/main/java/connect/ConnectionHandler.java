    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        switch (message.getCommand()) {
            ...
            case ADDR:
                ...
                List<NetAddr> addresses = addr.getList();

                if (isSendGetaddr && addresses.size() >= 1) {

                    for (NetAddr netAddr : addresses) {
                    ...
                    if (addresses.size() <= 100) {
                        for (NetAddr address : addresses) {
                            ConnectionManager.map.insert(address, bundle);
                        }
                        logger.info("[Monitor] add addresses size = {}", addresses.size());

                    } else {
                        logger.info("[Monitor] {} discard.", addresses.size());
                    }
